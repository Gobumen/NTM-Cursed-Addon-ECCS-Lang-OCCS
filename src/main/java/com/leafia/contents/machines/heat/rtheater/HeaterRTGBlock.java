...
public class HeaterRTGBlock extends AddonBlockDummyable implements ITooltipProvider, ILookOverlay {
    ...
    public void printHook(Pre event, World world, BlockPos pos) {
        ...
        int y = pos.getY();
        int z = pos.getZ();
        int[] p = this.findCore(world, x, y, z);

        if (p == null)
            return;

        TileEntity te = world.getTileEntity(new BlockPos(p[0], p[1], p[2]));

        if (!(te instanceof HeaterRTGTE))
            return;

        HeaterRTGTE heater = (HeaterRTGTE) te;
